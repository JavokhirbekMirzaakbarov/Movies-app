...
import { genres, Movie } from "../../constants";
...
interface MovieFormProps {
  initialValues: Movie;
  onSubmit: (movie: Movie) => Promise<number>;
}
...
const MovieForm: React.FC<MovieFormProps> = ({ initialValues, onSubmit }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      vote_average: Yup.number().min(0, "Minimum rating is 0").max(100, "Maximum rating is 100"),
      genres: Yup.lazy((val) =>
        Array.isArray(val)
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      if (response === 200) {
        ...
        dispatch(editMovie({ id: initialValues.id, movie: values }));
      ...
      <Box id="add-movie-modal-description">
        <Grid container spacing={2}>
          ...
          <Grid item xs={6} md={4}>
            ...
            <Input {...formik.getFieldProps("release_date")} id="date" type="date" fullWidth />
            {formik.touched.release_date && formik.errors.release_date && (
              <Typography sx={{ color: "red" }}>{formik.errors.release_date}</Typography>
            )}
          </Grid>
          <Grid item xs={6} md={8}>
            <TextField
            ...
            {formik.touched.poster_path && formik.errors.poster_path && (
              <Typography sx={{ color: "red" }}>{formik.errors.poster_path}</Typography>
            )}
          </Grid>
          <Grid item xs={6} md={4}>
            <TextField
            ...
            {formik.touched.vote_average && formik.errors.vote_average && (
              <Typography sx={{ color: "red" }}>{formik.errors.vote_average}</Typography>
            )}
          </Grid>
          <Grid item xs={6} md={8}>
            <Select
            ...
            >
              ...
              {genres.map((genre) => (
                <MenuItem key={genre} value={genre}>
                  <Checkbox checked={formik.values.genres.includes(genre as never)} />
                  <ListItemText primary={genre} />
                </MenuItem>
              ))}
            </Select>
            {formik.touched.genres && formik.errors.genres && (
              <Typography sx={{ color: "red" }}>{formik.errors.genres}</Typography>
            )}
          </Grid>
          <Grid item xs={6} md={4}>
            <TextField
          ...
          <Grid item xs={6} md={8}>
            ...
            {formik.touched.tagline && formik.errors.tagline && (
              <Typography sx={{ color: "red" }}>{formik.errors.tagline}</Typography>
            )}
          </Grid>
          <Grid item xs={6} md={4}>
            <TextField
          ...
          <Grid item xs={12} md={12}>
            ...
            {formik.touched.overview && formik.errors.overview && (
              <Typography sx={{ color: "red" }}>{formik.errors.overview}</Typography>
            )}
          </Grid>
          <Grid item xs={12} md={12}>
            {snackbar.message && (
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
          <Button type="submit" sx={{ margin: "10px" }} size="large" variant="contained">
            Submit
          </Button>
          <Button
